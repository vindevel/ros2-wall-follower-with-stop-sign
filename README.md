...
본 프로젝트는 **2025년 자율주행로봇 전문가 과정(광운대학교 공학교육혁신센터)**의 일환으로 진행되었다. 총 40시간의 집중 교육 과정 중 마지막 2일 동안 구현되었다.

ROS2 환경에서 LiDAR 데이터를 이용한 벽 따라가기와 YOLOv5 기반의 Stop Sign 표지판 인식을 결합한 통합 제어 시스템이다. 실제 로봇의 하드웨어 제약(GPU 부재)을 극복하기 위해 ONNX Runtime을 활용한 CPU 최적화 추론을 구현했으며 Docker 기반의 시뮬레이션 환경과 실제 로봇 플랫폼(Storagy)에서 동작을 확인했다.
...
- YOLOv5n가 Stop Sign 표지판을 인식하면 즉각 정지 명령을 수행
